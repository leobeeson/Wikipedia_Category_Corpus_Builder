...
get_subcats <- function(category){
  subcats_out <- pages_in_category(language = "en", project = "wikipedia", categories = category,
                                   type = "subcat", limit = 500)
  subcats_list <- unnest_list(subcats_out, "categorymembers")
  subcats_df <- bind_rows(subcats_list)
  subcats <- str_extract(string = subcats_df$title, pattern = "(?<=Category:).*$")
  return(subcats)
}
...
drill_down_domain <- function(domain, blacklist_subcats, blacklist_substrings, degrees){
  edge_list <- data.frame(super=character(), sub=character(), stringsAsFactors = FALSE)
  subcats <- c()
  ...
  while(degree_counter <= degrees){
    ...
    for(subcat in subcats){
      ...
      if(subcats_counter %% 50 == 0){
        ...
        message(paste0("Totla Remaining Subcats: ",length(subcats)),"\r",appendLF=TRUE)
        message(paste0("Ellapsed Time: ",(Sys.time() - t1)),"\r",appendLF=TRUE)
        flush.console()
        subcat_log <- c()
      }
     
      subcats_temp <- get_subcats(subcats[1])
      if(length(subcats_temp) == 0){
        subcats <- subcats[-1]
        next
      }
